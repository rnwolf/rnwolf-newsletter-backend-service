// tests/metrics.test.ts - Comprehensive metrics testing
...
describe(`Metrics System Tests (${TEST_ENV} environment)`, () => {
  beforeEach(async () => {
    if (config.setupDatabase) {
      ...
      `).bind('metrics-test-1@example.com', new Date().toISOString()).run();
      
      await env.DB.prepare(`
        INSERT INTO subscribers (email, subscribed_at, unsubscribed_at, ip_address, country)
        VALUES (?, ?, NULL, '192.168.1.2', 'US')
      `).bind('metrics-test-2@example.com', new Date().toISOString()).run();
    }
  });
